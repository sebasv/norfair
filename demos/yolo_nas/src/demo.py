...
class YOLO_NAS:
    def __init__(self, model_name: str, device: Optional[str] = None):
        ...
        # load model
            ...
            self.model = super_gradients.training.models.get("yolo_nas_l", pretrained_weights="coco").cuda()
...
def yolo_detections_to_norfair_detections(
    ...
    if track_points == "centroid":
        ...
        for label, conf, bbox_yolo in zip(labels, confidence, bboxes, strict=False):
            bbox = np.array(
                [
            ...
            norfair_detections.append(Detection(points=bbox, scores=scores, label=class_names[int(label)]))
...
parser.add_argument("--model-name", type=str, default="yolovnas", help="YOLOv5 model name")
parser.add_argument("--img-size", type=int, default="720", help="YOLO_nas inference size (pixels)")
parser.add_argument(
    "--conf-threshold",
    type=float,
    default="0.25",
...
parser.add_argument("--device", type=str, default="cuda", help="Inference device: 'cpu' or 'cuda'")
parser.add_argument(
    "--track-points",
    type=str,
    default="bbox",
...
for input_path in args.files:
    ...
    distance_threshold = DISTANCE_THRESHOLD_BBOX if args.track_points == "bbox" else DISTANCE_THRESHOLD_CENTROID
